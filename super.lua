...
local function check_member_super(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      local text = '?? تـم تـفـعـيـل الـمـجـمـوعـه ??'
...
local function check_member_superrem(cb_extra, success, result)
  ...
  for k,v in pairs(result) do
    ...
    if member_id ~= our_id then
      ...
      save_data(_config.moderation.data, data)
      local text = '?? تـم تـعـطـيـل الـمـجـمـوعـه ??'
      return reply_msg(msg.id, text, ok_cb, false)
    end
  end
end
...
local function lock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'yes' then
    return 'تم??بالفعل قفل??مضاد السبام?\n\n ?? Order by :? @'..msg.from.username
  else
...
local function unlock_group_spam(msg, data, target)
  ...
  if group_spam_lock == 'no' then
    return ' تم ?? بالفعل فتح ?? مضاد ?? السبام ???\n\n ?? Order by :? @'..msg.from.username
  else
...
local function lock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'yes' then
    return 'الاضافه الجماعيه بالفعل ?? تم قفلها ??\n\n ?? Order by :? @'..msg.from.username
  else
...
local function unlock_group_rtl(msg, data, target)
  ...
  if group_rtl_lock == 'no' then
    return 'الاضافه الجماعيه بالفعل ?? تم فتحها ???\n\n ?? Order by :? @'..msg.from.username
  else
...
local function lock_group_sticker(msg, data, target)
  ...
  if group_sticker_lock == 'yes' then
    return 'الملصقات بالفعل ?? تم قفلها ???\n\n ?? Order by :? @'..msg.from.username
  else
...
local function unlock_group_contacts(msg, data, target)
  ...
  if group_contacts_lock == 'no' then
    return 'جهات الاتصال بالفعل ?? تم فتحها ???\n\n ?? Order by :? @'..msg.from.username
  else
...
local function disable_strict_rules(msg, data, target)
  ...
  if group_strict_lock == 'no' then
    return 'تم بالفعل ?? فتح ?? جميع الاعدادات ????\n\n ?? Order by :? @'..msg.from.username
  else
...
local function set_rulesmod(msg, data, target)
  ...
  save_data(_config.moderation.data, data)
  return 'تم ?? وضع القوانين ?? في المجموعه ????'
end
...
local function set_public_membermod(msg, data, target)
  if not is_momod(msg) then
    return "??? لا يمكنك التحكم ?? في البوت للادمنيه ????"
  end
...
local function demote_admin(receiver, member_username, user_id)
  ...
  if not data[group]['moderators'][tostring(user_id)] then
    return send_large_msg(receiver, member_tag_username..'هوه بالفعل ?? ضمن الاعضاء ????')
  end
...
local function demote2(receiver, member_username, user_id)
  ...
  data[group]['moderators'][tostring(user_id)] = nil
  save_data(_config.moderation.data, data)
  send_large_msg(receiver, member_username..'تم ?? تنزيلك ?? من الادمنيه لانك فاشل ??')
end
...
function get_message_callback(extra, success, result)
	...
	elseif get_cmd == "رفع اداري" then
		...
		if result.from.username then
            text = "@"..result.from.username.."تم ?? رفعك ?? في الاداره ??  شدلك محرك جـارجـر ????"
		else
            text = "[ "..user_id.." ]تم ?? رفعك ?? في الاداره ?? شدلك محرك جـارجـر  ????"
		end
	...
	elseif get_cmd == "تنزيل اداري" then
		...
		if result.from.username then
            text = "@"..result.from.username.."تم ?? تنزيلك ?? من الاداره ?? لانك فاشل "
		else
            text = "[ "..user_id.." ] تم ?? تنزيلك ?? من الاداره ?? لانك فاشل ???"
		end
savelog(msg.to.id, name_log.." ["..msg.from.id.."] demoted: ["..user_id.."] from admin by reply")
    ...
    elseif get_cmd == "رفع المدير" then
        ...
        local channel_id = 'channel#id'..result.to.peer_id
            ...
            if result.from.username then
                text = "@"..result.from.username.." [ "..result.from.peer_id.." ] تم ?? رفعك مدير ?? كبد حياتي ????"
            else
                text = "[ "..result.from.peer_id.." ] تم ?? رفعك مدير ?? كبد حياتي ????"
            end
            send_large_msg(channel_id, text)
        end
	...
	elseif get_cmd == 'mute_user' then
		...
		if is_muted_user(chat_id, user_id) then
			...
			send_large_msg(receiver, "["..user_id.."] ?? جراره ??  راح الكتم منك يلا دردش ??")
    ...
    if vusername == member or vpeer_id == memberid then
     ...
     local sender = cb_extra.msg.from.id
      ...
      if is_momod2(user_id, channel_id) and not is_admin2(sender) then
        return send_large_msg("channel#id"..channel_id, "? لا تلعب بكيفك لايمكنك طرد الادمن او المديرَ??")
      end
      if is_admin2(user_id) then
        return send_large_msg("channel#id"..channel_id, "? لا تلعب  بكيفك لايمكنك طرد الاداري??")
      end
      if v.username then
        text = ""
	...
	for k,v in pairs(result) do
		...
		if vusername == member or vpeer_id == memberid then
			...
			if group_owner then
				...
				if result.username then
                    text = member_username.." ["..v.peer_id.."]ولا يهمك تم ?? رفعك مدير ?? سفنجه كلبي ??"
				else
                    text = "["..v.peer_id.."]ولا يهمك تم ?? رفعك مدير ?? سفنجه كلبي ??"
				end
			end
		elseif memberid and vusername ~= member and vpeer_id ~= memberid then
			...
			if group_owner then
                ...
                text = "["..memberid.."]ولا يهمك تم ?? رفعك مدير ?? سفنجه كلبي ??"
			end
		end
	end
 end
	...
	local data = load_data(_config.moderation.data)
		if matches[1] == 'تفعيل' and not matches[2] then
			...
			if is_super_group(msg) then
                return reply_msg(msg.id, 'المجموعه بالتأكيد ?? تم تفعيلها ????', ok_cb, false)
			end
		...
		if matches[1] == 'تعطيل' and is_admin1(msg) and not matches[2] then			if not is_super_group(msg) then
                return reply_msg(msg.id, 'المجموعه بالتأكيد ?? تم تعطيلها ????', ok_cb, false)
			end
		...
		if matches[1] == "كشف بوت" and is_momod(msg) then
            member_type = 'تم ?? الكشف عن البوتات في المجموعه ??'
		...
		if matches[1] == 'ايدي' then
			...
			elseif msg.text:match("@[%a%d]") then
                ...
                return "ايدي الـمـجـمـوعـه ??"..string.gsub(msg.to.print_name, "_", " ")..": "..msg.to.id
			end
		end
		...
		if matches[1] == 'تغير الرابط' and is_momod(msg)then
			...
			local receiver = get_receiver(msg)
				if success == 0 then
                    send_large_msg(receiver, 'عذر? ??لا يمكنك ??تغيير الرابط ???\nالمجموعه ?? ليست من صنع البوت ??\n\nيرجى ?? استخدام ??الرابط الخاص بها ?? في اعدادات المجموعه ??')
		...
		if matches[1] == 'ضع رابط' and is_owner(msg) then
			...
			save_data(_config.moderation.data, data)
            return 'يرجى ارسال ?? الرابط ??الخاص بك ????'
		end
	 ...
	 if matches[1] == "ضع اسم" and is_momod(msg) then
        ...
        if matches[1] == "ضع معرف" and is_admin1(msg) then
            local function ok_username_cb (extra, success, result)
                local receiver = extra.receiver
                if success == 1 then
                    send_large_msg(receiver, "تم ?? وضـع  ?? معرف ?? للمجموعه ????\n\n?? انظر الى الحول ?? لتشاهد تغيرات ?? المجموعه ????")
                elseif success == 0 then
                    send_large_msg(receiver,"فشل ?? تعين ? معرف ?? المجموعه ?????\nUsername may already be taken.\n\nNote: Username can use a-z, 0-9 and underscores.\nMinimum length is 5 characters.")
                end
            end
            local username = string.gsub(matches[2], '@', '')
        ...
        if matches[1] == 'مسح' then
            ...
            if not is_momod(msg) then
                return "?? لتلعب بكيفك ?? فقط المدير ?? يحق له ذلك ??"
            end
            if matches[2] == 'الادمنيه' then
                if next(data[tostring(msg.to.id)]['moderators']) == nil then
                    return 'عذر? ?? لا يوجد ?? ادمنيه ليتم مسحهم ????'
                end
                for k,v in pairs(data[tostring(msg.to.id)]['moderators']) do
            ...
            if matches[2] == 'القوانين' then
                ...
                if data[tostring(msg.to.id)][data_cat] == nil then
                    return "عذر? ?? لا يوجد ?? قوانين ?? في المجموعه ?? ليتم مسحها ????"
                end
            ...
            if matches[2] == 'الوصف' then
                ...
                local data_cat = 'description'
                if data[tostring(msg.to.id)][data_cat] == nil then
                    return 'عذر? ?? لا يوجد وصف ?? في المجموعه ?? ليتم مسحه ??'
                end
                data[tostring(msg.to.id)][data_cat] = nil
                save_data(_config.moderation.data, data)
        ...
        if matches[1] == 'قفل' and is_momod(msg) then
            ...
            local msg_type = ''
                if not is_muted(chat_id, msg_type..': yes') then
                    ...
                    return ""..msg_type..'الصوتيات بالفعل ?? تم قفلها ???\n\n ?? Order by :? @'..msg.from.username
                end
            end
           if matches[2] == 'الصور' then
            local msg_type = ''
                if not is_muted(chat_id, msg_type..': yes') then
                    ...
                    return ""..msg_type..'الصور بالفعل ?? تم قفلها ???\n\n ?? Order by :? @'..msg.from.username
                end
            end
            ...
            local msg_type = ''
                if not is_muted(chat_id, msg_type..': yes') then
                    ...
                    return ""..msg_type..'الفيديوهات بالفعل ?? تم قفلها ????\n\n ?? Order by :? @'..msg.from.username
                end
            end
            ...
            local msg_type = ''
                if not is_muted(chat_id, msg_type..': yes') then
                    savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: mute "..msg_type)
                    mute(chat_id, msg_type)
                    return msg_type..'تم ?? قفل ?? الدردشة ???\n\n ?? Order by :? @'..msg.from.username
                else
                    return ""..msg_type..'الدردشة بالفعل ?? تم قفلها ???\n\n ?? Order by :? @'..msg.from.username
                end
            end
        ...
        if matches[1] == 'فتح' and is_momod(msg) then
            ...
            local msg_type = ''
                if is_muted(chat_id, msg_type..': yes') then
                    ...
                    return ""..msg_type..'تم بالفعل ?? فتح ?? الصوتيات ???\n\n ?? Order by :? @'..msg.from.username
                end
            end
            if matches[2] == 'الصور' then
            local msg_type = ''
                if is_muted(chat_id, msg_type..': yes') then
                    ...
                    return ""..msg_type..'الصور ?? بالفعل ?? تم فتحها ???\n\n ?? Order by :? @'..msg.from.username
                end
            end
            if matches[2] == 'الفيديو' then
            local msg_type = ''
                if is_muted(chat_id, msg_type..': yes') then
                    savelog(msg.to.id, name_log.." ["..msg.from.id.."] set SuperGroup to: unmute "..msg_type)
                    unmute(chat_id, msg_type)
                    return msg_type..'تم ?? فتح ?? الفيديوهات ????\n\n ?? Order by :? @'..msg.from.username
                else
                    return ""..msg_type..'تم بالفعل ?? فتح ?? الفيديوهات ???\n\n ?? Order by :? @'..msg.from.username
                end
            end
            if matches[2] == 'الصور المتحركه' then
            local msg_type = ''
                if is_muted(chat_id, msg_type..': yes') then
                    ...
                    return ""..msg_type..'تم بالفعل ?? فتح ?? الصور المتحركه ???\n\n ?? Order by :? @'..msg.from.username
                end
            end
            if matches[2] == 'الفايلات' then
            local msg_type = ''
                if is_muted(chat_id, msg_type..': yes') then
                    ...
                    return ""..msg_type..'الفايلات بالفعل ?? تم فتحها ???\n\n ?? Order by :? @'..msg.from.username
                end
            end
            if matches[2] == 'الدردشه' then
            local msg_type = ''
